...
// J_k J型指令的四个立即数部分
#define immJ() do { *imm = SEXT((BITS((uint32_t)0, 31, 12)) | (BITS(i, 31, 31) << 19) | BITS(i, 30, 21) | (BITS(i, 20, 20) << 10) | BITS(i, 19, 12), 20);} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 1100111", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & (~(word_t)0x01););         // 跳转并寄存器链接
  ...
  INSTPAT("??????? ????? ????? ??? ????? 1101111", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm*2);
